...
export class DeviceInfoPage implements OnInit {
  ...
  async toggleChange($event) {
    ...
    Promise.all([this.storage.get('token'), this.storage.get('refresh'), this.storage.get('iduser'), this.storage.get('namedevice')]).then(values => {

      if (values[0] && values[0] !== "") {
        //Appel API
        this.apiservice.apiSetLed(values[0], values[1], values[2], values[3], this.ledvalue)
          .subscribe(valRetour => {
